    ...
    return (
      <div className="animate-fade-up">
        ...
        <div className="bg-white/5 border border-white/5 rounded-2xl p-6 overflow-hidden">
          <div className="grid grid-cols-7 text-[10px] text-white/30 tracking-widest mb-4 px-1 font-mono-custom uppercase">
            <span>COLUMN</span>
            <span>TYPE</span>
            <span>MIN</span>
            <span>MAX</span>
          ...
          <div className="space-y-1">
            {profile.column_details.map((col, i) => {
              ...
              return (
                <div key={i} className="grid grid-cols-7 py-3 border-t border-white/5 text-sm items-center hover:bg-white/[0.02] transition-colors">
                  <span className="text-white/80 font-display font-bold">{name}</span>
                  <span>
                    <span className="bg-white/10 text-white/60 text-[9px] px-2 py-0.5 rounded font-mono-custom uppercase border border-white/5">
                      {type}
                    </span>
                  </span>
                  <span className="text-white/40 font-mono-custom text-xs">{col.min ?? "—"}</span>
                  <span className="text-white/40 font-mono-custom text-xs">{col.max ?? "—"}</span>
                  <span className="text-white/40 font-mono-custom text-xs">{col.mode ?? "—"}</span>
                  <span className="text-white/40 font-mono-custom text-xs">{col.mean != null ? Number(col.mean).toFixed(1) : "—"}</span>
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
                      <div className="h-full bg-amber-500/60" style={{ width: `${Math.min(nullPct, 100)}%` }} />
                    </div>
                    <span className="text-white/30 text-[10px] font-mono-custom w-8 text-right">{nullPct}%</span>
                  </div>
                </div>
              )
            })}
      ...
      <div className="animate-fade-up space-y-12">
        {Object.keys(featureImportance).length > 0 && (
          <div className="bg-white/5 border border-white/5 rounded-2xl p-8">
            <p className="text-[10px] text-white/30 tracking-widest mb-6 font-mono-custom uppercase">
                ...
                .map(([col, val]) => (
                  <div key={col} className="bg-white/5 p-4 rounded-xl border border-white/5">
                    <div className="flex justify-between text-xs mb-2">
                      <span className="text-white/80 font-display font-bold truncate pr-2">{col}</span>
                      <span className="text-amber-500 font-mono-custom">{Math.round((val / maxImportance) * 100)}%</span>
                    </div>
        ...
        <div className="bg-white/5 border border-white/5 rounded-2xl p-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              <p className="text-[10px] text-white/30 tracking-widest font-mono-custom uppercase mb-1">CORRELATION MATRIX</p>
              <p className="text-white/50 text-xs font-display">Discovery of relationships between numeric features</p>
            </div>
            <div className="inline-flex rounded-full border border-white/15 bg-[#0f0f0f] p-1">
              {["pearson", "spearman"].map((m) => (
                <button
                  ...
                  className={`px-6 py-2 text-[10px] rounded-full capitalize transition-all font-display font-bold ${
            ...
            <table className="text-[10px] border-separate border-spacing-1 mx-auto">
              <thead>
                <tr>
                  ...
                  {columns.map(col => (
                    <th key={col} className="px-2 py-3 text-white/30 font-mono-custom font-normal whitespace-nowrap min-w-[64px] max-w-[64px] overflow-hidden text-ellipsis" title={col}>
                      <div className="rotate-[-45deg] translate-y-2">{col.length > 8 ? col.substring(0, 6) + ".." : col}</div>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {columns.map(row => (
                  <tr key={row}>
                    <td className="sticky left-0 bg-[#161616] z-10 pr-4 py-2 text-white/40 text-right whitespace-nowrap font-mono-custom text-[11px] font-bold" title={row}>
                      {row}
                    </td>
                    {columns.map(col => {
                      ...
                      return (
                        <td
                          key={col}
                          className={`w-14 h-14 text-center font-mono-custom text-[10px] font-bold rounded-lg transition-all hover:scale-110 cursor-default shadow-inner ${getCellColor(val)} ${isSelf ? "opacity-20" : "opacity-100"}`}
                        >
                          {val.toFixed(2)}
                        </td>
                      )
            ...
            return (
              <div key={cluster.id} className={`bg-white/5 rounded-2xl p-6 border-l-4 ${color.border} hover:bg-white/[0.08] transition-colors`}>
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className={`font-display font-black text-2xl ${color.text}`}>Segment {String.fromCharCode(65 + i)}</h3>
                    <p className="text-white/30 text-xs font-mono-custom">{cluster.size} rows · {pct}% of dataset</p>
                  </div>
                </div>
                <div className="space-y-4">
                  {cluster.stats.numeric && (
                    <div>
                      <p className="text-[10px] text-white/20 tracking-widest font-mono-custom uppercase mb-2">Averages</p>
                      <div className="space-y-1.5">
                        {Object.entries(cluster.stats.numeric).map(([col, val]) => (
                          <div key={col} className="flex justify-between text-[11px]">
                            ...
                            <span className="font-mono-custom text-amber-400/80">{val?.toFixed ? val.toFixed(2) : val}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {cluster.stats.categorical && (
                    <div>
                      <p className="text-[10px] text-white/20 tracking-widest font-mono-custom uppercase mb-2">Dominant</p>
                      <div className="space-y-1.5">
                        {Object.entries(cluster.stats.categorical).map(([col, val]) => (
                          <div key={col} className="flex justify-between text-[11px]">
                            <span className="text-white/40">{col}</span>
                            <span className="font-mono-custom text-green-400/80">{val}</span>
                          </div>
                        ))}
                      </div>
                    </div>
